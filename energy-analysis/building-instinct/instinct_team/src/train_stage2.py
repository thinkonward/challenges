...
def get_args():
    parser = argparse.ArgumentParser(description='Training Script for Stage 2 Model')
...
class Stage2ExtBuildingDataset(Dataset):
    def __init__(self, df, mode,target_cols,features_type,args=None):
        self.df = df.reset_index().astype(object)
        ...
        for target_col in self.target_cols:
            with open(f'{self.args.data_dir}/Label_encoded/{target_col}_label_encoder.pkl', 'rb') as f:
                ...
                if self.mode == 'test':
                    ...
                    self.targets.append(le.transform(df[target_col].astype(str)))
        if self.mode == 'train':
    ...
    def __getitem__(self, index):
        ...
        if self.target_cols[0].endswith("res"):
            df = pd.read_parquet(f'{self.args.data_dir}/external/features/res/{state}/{id}-0.parquet',engine='fastparquet')
        else:
            df = pd.read_parquet(f'{self.args.data_dir}/external/features/com/{state}/{id}-0.parquet',engine='fastparquet')
...
def run(fold,df_ext,target_cols,criterion,kernel_type,args):
    ...
    exttrain_ = df_ext[df_ext['fold'] != fold]
    extvalid_ = df_ext[df_ext['fold'] == fold]
    dataset_train = Stage2ExtBuildingDataset(exttrain_, 'train',target_cols,args.features_type,args=args)
    num_classes = dataset_train.num_classes
    dataset_valid = Stage2ExtBuildingDataset(extvalid_, 'valid',target_cols,args.features_type,args=args)
...
def main():
    args=get_args()
    kernel_type = f'stage2_{args.exp_id}_{args.target_type}'
    ...
    if args.target_type =='res':
        df_ext = pd.read_parquet(args.data_dir+'/labels/external_res_fold10.parquet',engine='pyarrow')
        target_cols = [col for col in df_ext.columns if col.endswith("res") ]
        
    else:
        df_ext = pd.read_parquet(args.data_dir+'/labels/external_com_fold10.parquet',engine='pyarrow')
        target_cols = [col for col in df_ext.columns if col.endswith("com") ]
        
    criterion = MultiLabelLoss(len(target_cols))
    for fold in range(1):
        run(fold,df_ext,target_cols,criterion,kernel_type,args)
