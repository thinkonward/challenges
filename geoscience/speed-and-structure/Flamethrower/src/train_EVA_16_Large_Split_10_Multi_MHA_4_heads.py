...
from models.model_EVA_16_Large_Split_10_Multi_MHA_4_heads import (
...
class Args:
    ...
    backbone_ckpt = ""
    name = "EVA_16_Large_Split_10_Multi_MHA_4_heads"
...
def train_and_validate(args, train_df, valid_df):
    """
    Train and validate the EVA16 model on provided training and validation datasets.

    This function:
    - Sets random seeds for reproducibility.
    - Initializes the model and its exponential moving average (EMA) version.
    - Prepares DataLoaders for training and validation.
    - Defines the loss function, optimizer, and a two-stage learning rate scheduler (cosine annealing followed by constant LR).
    - Runs training and validation epochs, tracking metrics and saving the best model checkpoint based on validation MAPE.
    - Implements early stopping.

    Args:
        args (Namespace): Configuration containing hyperparameters and paths.
        train_df (pd.DataFrame): DataFrame with training dataset metadata.
        valid_df (pd.DataFrame): DataFrame with validation dataset metadata.
...
def main():
    """
    Parse command-line arguments and start training/validation for specified fold.
    """
