...
def dummy_prediction(input_volume):
    ...
    """
    prediction = np.random.choice([0, 1], size=input_volume.shape, p=[.97, 0.03])
    return prediction
...
def get_submission_score(gt_submission_path, prediction_submission_path, mask_shape=(300, 300, 1259)):
    # load submissions
    ...
    for sample_id in gt_submission.keys():
        ...
        if gt_coordinates.shape[0]>0:
            gt_mask[gt_coordinates[:,0], gt_coordinates[:,1], gt_coordinates[:,2]] = 1
        ...
        if pred_coordinates.shape[0]>0:
            pred_mask[pred_coordinates[:,0], pred_coordinates[:,1], pred_coordinates[:,2]] = 1
...
def get_sampling_freq_window_length(
    seis_vol: xr.DataArray
) -> Tuple[float, float]:
