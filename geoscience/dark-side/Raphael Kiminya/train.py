...
paths_trn = [p for p in VOLUME_PATHS if p not in paths_val] 
...
class SeisDatasetTrain(Dataset):
  ...
  def __getitem__(self, idx):
    ...
    if np.random.rand()<0.9:
        depth_candidates = np.where(y.sum(axis=(1,2))>0)[0]
...
wts = torch.load('output/model-p0-epoch=499.ckpt')['state_dict']
model.load_state_dict(wts)

checkpoint_callback = L.pytorch.callbacks.ModelCheckpoint(save_top_k=20,monitor="val_dice",mode="max",dirpath="output",filename="model-p1-{epoch:02d}")
