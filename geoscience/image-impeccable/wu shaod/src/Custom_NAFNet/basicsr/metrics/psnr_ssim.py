...
def calculate_custom_ssim(img1, img2):
    img1 = img1.detach().cpu().numpy().squeeze(0)
    img2 = img2.detach().cpu().numpy().squeeze(0)
    scores = [structural_similarity(img1[i,...], img2[i,...], data_range=255) 
              for i in range(img1.shape[0])]
    np.mean(scores)
    return np.mean(scores)

# def calculate_custom_ssim(imgs1, imgs2):
#     imgs1 = imgs1.detach().cpu().numpy().squeeze()
#     imgs2 = imgs2.detach().cpu().numpy().squeeze()
